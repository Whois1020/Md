...
const tags = {
  ...
  serbot: textCyberpunk('vendedores')
}

const defaultMenu = {
  before: `
—͟͟͞͞ ♱   *Registro Arceus MD* »
> 🪐 𝙉𝙤𝙢𝙗𝙧𝙚   » %name
> ⚙️ 𝙉𝙞𝙫𝙚𝙡     » %level
> ⚡ 𝙀𝙭𝙥        » %exp / %maxexp
> 🌐 𝙈𝙤𝙙𝙤      » %mode
...
🤖 » 𝐌𝐄𝐍𝐔 «
👑 » 𝗢𝗽𝗲𝗿𝗮𝗱𝗼𝗿:—͟͟͞͞ *Whois* 𖣘 «
%readmore
`.trimStart(),
  header: '\n⧼⋆꙳•〔 ♱ %category 〕⋆꙳•⧽',
  body: '> 𖣘 %cmd',
  footer: '╰⋆꙳•❅‧*₊⋆꙳︎‧*❆₊⋆╯',
  after: '\n⌬ 𝗠𝗘𝗡𝗨 🧬 - Sistema ejecutado con éxito.'
}
...
const defaultThumb = await fetchBuffer('https://o.uguu.se/MJVbgdQD.jpg')
...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  await conn.sendMessage(m.chat, {
    ...
    caption: text,
    footer: '🧠 ARCEUS WHOIS ☘️',
    buttons: [
      { buttonId: `${usedPrefix}menurpg`, buttonText: { displayText: '🏛️ M E N U R P G' }, type: 1 },
      { buttonId: `${usedPrefix}code`, buttonText: { displayText: '🕹 C O M P R A R' }, type: 1 }
    ],
    contextInfo: {
      externalAdReply: {
        title: menuMedia.menuTitle || 'Arceus Whois ⚡',
        body: 'ִ┊࣪ ˖Dev : Whois ♱',
